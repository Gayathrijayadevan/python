...
while True:
    ...
    if ch == 1:
        ...
        # cursor.execute("select employee.id from employee inner join dls on ")
        cursor.execute("SELECT employees.emp_id,employees.name,employees.age,employees.email,employees.number,employees.position,employees.salary,dtls.address FROM employees right JOIN dtls on employees.emp_id=dtls.emp_id")            
        data=cursor.fetchall()
        # print('{:<10}{:<10}{:<10}{:<20}{:<20}{:<20}{:<10}{:<10}'.format('id', 'name', 'age', 'email','number' ,'position','salary','address'))
        # print('-' * 110)
        for i in data:
                # print("{:<10}{:<10}{:<10}{:<20}{:<20}{:<20}{:<10}{:<10}".format(i[0], i[1], i[2], i[3], i[4],i[5],i[6],i[7]))
                print(i)
